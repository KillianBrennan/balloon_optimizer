...
c     Name of file with balloon ascent characteristics
      character*180 balloon_file
      parameter    (balloon_file = 'balloon' )
...
c     Meteorological fields
      ...
      real,allocatable, dimension (:)     :: tempt0,tempt1   ! 3d temperature field
...
c     Balloon ascent characteristics
      character*80                           balloon_type(1000) ! z (height) or t (time)
      real                                   balloon_low (1000) ! z or t value
      real                                   balloon_hig (1000) 
      real                                   balloon_w   (1000) ! vertical speed (in m/s)
      integer                                nballoon
      real,allocatable, dimension (:)     :: balloon
      integer,allocatable, dimension (:)  :: balloon_state      ! State of the balloon (sising=1,sinking=2)
...
c     Get optional arguments
     ...
     >       '---- OPTIONAL PARAMETERS --------------------------------'
           ...
           i = i + 1
           test = 1
           print*,'  optional argument -test set to : ',1
        endif
...
c     Read balloon velocity file
      INQUIRE(FILE=balloon_file, EXIST=file_exists)
      if ( file_exists.eqv..false. ) then
        nballoon = 0
      else
        nballoon = 0
        open(10,file=balloon_file)
  11      nballoon = nballoon + 1
          read(10,*,end=21) balloon_type(nballoon),
     >                      balloon_low(nballoon),balloon_hig(nballoon),
     >                      balloon_w(nballoon)
          goto 11
  21      continue
          nballoon = nballoon - 1            
     ...
     >       '---- BALLOON --------------------------------------------'
         print* 
         do i=1,nballoon
           write(*,'(a3,f10.2,f10.2,f10.2)' ) 
     >        balloon_type(i),balloon_low(i),balloon_hig(i),
     >        balloon_w(i)
         enddo    
         print*
         do i=1,nballoon
           if ( ( balloon_type(i).eq.'ta' ).or.
     >          ( balloon_type(i).eq.'td' ) )
     >     then
              call hhmm2frac(balloon_low(i),tfrac)
              balloon_low(i) = nint ( tfrac * 60. )
              call hhmm2frac(balloon_hig(i),tfrac)
              balloon_hig(i) = nint ( tfrac * 60. )
           endif
         enddo
      endif
...
c     For balloon trajectories, add balloon speed 
      if ( nballoon.gt.0 ) then
           ncol = ncol + 1
      endif
...
C     Get memory for trajectory arrays
      ...
      allocate(balloon(ntra),stat=stat)
      if (stat.ne.0) print*,'*** error allocating array balloon ***'  ! Balloon ascent/descent vertical speed
      allocate(balloon_state(ntra),stat=stat)
      if (stat.ne.0) print*,'*** error allocating balloon_state ***'  ! Balloon ascnet/descent state
...
C     Allocate memory for some meteorological arrays
      ...
      allocate(tempt0(nx*ny*nz),stat=stat)
      if (stat.ne.0) print*,'*** error allocating array tempt0 ***' ! Temperature
      allocate(tempt1(nx*ny*nz),stat=stat)
      if (stat.ne.0) print*,'*** error allocating array tempt1 ***'
...
c     Read wind fields and vertical grid from first file
     ...
     >        tload,pollon,pollat,p3t1,spt1,nz,ak,bk,stagz,timecheck)
      ...
      varname='T'                                      ! T
      call input_wind 
     >    (fid,varname,tempt1,tload,stagz,mdv,
     >     xmin,xmax,ymin,ymax,dx,dy,nx,ny,nz,timecheck)
...
c         Get the balloon ascent/descent speed
          do i=1,ntra
              balloon(i) = 0.
              
c             Balloon state (1): rising
              if ( balloon_state(i).eq.1 ) then
                do k=1,nballoon
                 if ( balloon_type(k).eq.'pa' ) then
                    if ( ( pp0(i).le.balloon_low(k) ).and.
     >                   ( pp0(i).ge.balloon_hig(k) ) )
     >              then
                        balloon(i) = balloon_w(k)
                    endif
                 endif
                 if ( balloon_type(k).eq.'ta' ) then
                    if ( ( time.ge.balloon_low(k) ).and.
     >                   ( time.le.balloon_hig(k) ) )
     >              then
                        balloon(i) = balloon_w(k)
                    endif
                 endif
                 if ( balloon_type(k).eq.'pc' ) then
                    if ( ( pp0(i).le.balloon_low(k) ).or.
     >                   ( pp0(i).le.balloon_hig(k) ) )
     >              then
                        balloon(i)       = 0.
                        balloon_state(i) = 2
                    endif
...
c             Balloon state (2): descending
              if ( balloon_state(i).eq.2 ) then
                do k=1,nballoon
                 if ( balloon_type(k).eq.'pd' ) then
                    if ( ( pp0(i).le.balloon_low(k) ).and.
     >                   ( pp0(i).ge.balloon_hig(k) ) )
     >              then
                        balloon(i) = balloon_w(k)
...
c         Convert balloon speed from m/s to Pa/s (hydrostatic approximation)
          do i=1,ntra
              if ( balloon(i).ne.0. ) then

C                 Interpolate temperature to trajectory locarion
                  call get_index4 (xind,yind,pind,xx0(i),yy0(i),pp0(i),
     >                             reltpos0,p3t0,p3t1,spt0,spt1,3,
     >                             nx,ny,nz,xmin,ymin,dx,dy,mdv)
                  t0 = int_index4(tempt0,tempt1,nx,ny,nz,
     >                                  xind,yind,pind,reltpos0,mdv)

c                 Convert m/s to Pa/s
                  balloon(i) = -balloon(i) *
     >                        pp0(i) * 100. * gearth / ( rgas * t0 )
          ...
          do i=1,ntra
            if ( active(i).eq.2 ) then
              ...
              if ( abs( pp0(i) + 999. ).lt.eps ) then
                   ...
                   if ( nballoon.gt.0 ) then
                      traout(i,indt,4+ntrace+1) = balloon(i)
                   endif
              endif
           endif
          enddo
...
c         Avoid double-updating at time instances with input files
          if ( iloop.eq.(timeinc/ts) ) goto 101
...
c             Iterative Euler timestep (x0,y0,p0 -> x1,y1,p1)
     ...
     >               xmin,ymin,dx,dy,per,hem,nx,ny,nz,balloon(i),test)
...
c         Exit point for loop over time steps between input files         
 101      continue
...
c     Write trajectory file
      ...
      do i=1,ntrace
         vars(4+i) = tvar(i)
      enddo
      if ( nballoon.gt.0 ) vars(4+ntrace+1) = 'balloon'
      call wopen_tra
     >      (cdfid,cdfname,ntra,ntim,ncol,reftime,vars,outmode)
      call write_tra(cdfid,traout,ntra,ntim,ncol,outmode)
...
c     -------------------------------------------------------------------
     ...
     >                xmin,ymin,dx,dy,per,hem,nx,ny,nz,balloon,test)
...
c       Handle trajectories which cross the lower boundary (jump flag)
        if ( (balloon.gt.0.).and.(p1.gt.sp)) then
           left = 1
        elseif ((jump.eq.1).and.(p1.gt.sp)) then
           p1=sp-10.
        endif
